...
:END:
    ...
    -  [[#exwm][EXWM]]
    -  [[#meta][Meta]]
      -  [[#updating-from-inside-emacs][Updating from inside Emacs]]
...
Use column-enforce-mode instead.
...
#+begin_src emacs-lisp
  (akirak/require 'setup-typescript)
#+end_src
*** Vue
:PROPERTIES:
...
#+end_src
** Meta
*** Updating from inside Emacs
#+begin_src emacs-lisp
  (defun akirak/update-emacs-config ()
    (interactive)
    (let ((default-directory user-emacs-directory))
      (ansi-term (expand-file-name "update.bash" user-emacs-directory)
                 "*update config*")))
#+end_src
*** Ignore
:PROPERTIES:
