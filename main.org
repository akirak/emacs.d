...
*** Buffer management
#+begin_src emacs-lisp
  (akirak/require 'setup-switch-buffer)
#+end_src
...
#+begin_src emacs-lisp
  (use-package project
    ...
    :config
    (defmacro akirak/define-project-type (symbol dominating-file)
      (let ((func (intern (format "akirak/project-%s-root" (symbol-name symbol)))))
        `(progn
           ...
           (add-hook 'project-find-functions ',func t)
           (cl-defmethod project-roots ((project (head (quote ,symbol))))
             (list (cdr project)))
...
#+begin_src emacs-lisp
  (akirak/bind-specific-help "i" #'helm-info)
#+end_src
***** DevDocs
#+begin_src emacs-lisp
